...
function getGASdata(spreadsheet_data){
  ...
  // 各種設定
  g_doc_url = g_gas_data_arr[1][1]; // GoogleドキュメントのURLを取得
...
function doSearch(){
  ...
  for(let i = 1; i < g_gas_data_arr.length; i++){
    ...
    // キーワードのチェック（空白などをはじく関数の処理）
    // キーワードが続いた時の対応をどうするか。
    // 最初だけマークをつけて、次の文字も同じだったら処理せず1文字ずらす処理をいれればOKか。
    // if(true === check_keyword(keyword)){};
...
function do_add_mark(keyword, sentence){
  ...
  while(wordPlaceNum >= 0){
    ...
    if(add_circle === overlappingCheck){
      ...
      // キーワードが続いていた場合のチェックをここで入れる
      // 例えば、キーワードがaaで検索元の文章がaaaaaとなった場合は●a●a●a●aaとなってしまう。
      // 2文字以上のキーワードがある場合、対応を検討する必要がある。
...
function doRewriteDoc(doc, sentence){
  ...
  // ●を赤色に変えるには、パラグラフを操作するため全体を作り直す必要がある。
  // もしくはここで一文字ずつチェックして、●を赤色に変える処理を追加するか。。
  // editAsText().setForegroundColorを使う
